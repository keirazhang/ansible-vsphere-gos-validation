...
def extract_error_from_log(log_path):
    ...
    with open(log_path, 'r') as log_file:
        ...
        while (line_cursor < lines_count):
            ...
            if m:
                char_cursor = m.start()
                errors.append(escape_ansi(line))
                ...
                stack_pattern = f'(?<={left_pattern})(\s+.*)'
                next_line_cursor = line_cursor + 1
                while next_line_cursor < lines_count:
                    next_line = lines[next_line_cursor].strip()
                    stack_match = None
                    ...
                    if stack_match:
                        ...
                        errors.append(escape_ansi(next_line))
                        next_line_cursor += 1
                    else:
